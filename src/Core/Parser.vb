...
Friend Class SpotParser
    ...
    Friend Shared Function ParseSpot(ByVal xSpot As Spotlib.SpotEx, ByVal FontSize As Byte) As String
        ...
        If SpotHTM Is Nothing Then
            ...
            Dim sR As New StreamReader(Tools.SpotTheme)
        ...
        Return SpotHTM.Replace("[SN:NICK]", Utils.StripChars(My.Settings.Nickname)).Replace("[SN:AVATAR]", GetAvatar(xSpot.User.Avatar, xSpot.User.Modulus)).Replace("[SN:FONTSIZE]", CStr(FontSize)).Replace("[SN:FILESIZE]", CStr(xSpot.Filesize)).Replace("[SN:STAMP]", CStr(xSpot.Stamp)).Replace("[SN:MODULUS]", xSpot.User.Modulus).Replace("[SN:CAT]", Utils.HtmlEncode(Spotz.CatDesc(xSpot.Category))).Replace("[SN:PATH]", Tools.SettingsFolder.Replace(Chr(34), Chr(34) & Chr(34))).Replace("[SN:TAG]", Utils.HtmlEncode(Utils.StripChars(xSpot.Tag))).Replace("[SN:FROM]", Utils.HtmlEncode(Utils.StripChars(xSpot.Poster))).Replace("[SN:UNIQUE]", Utils.sIIF(xSpot.User.ValidSignature, Spotz.MakeUnique(xSpot.User.Modulus), "Onbekend")).Replace("[SN:WHITELIST]", Utils.sIIF(xSpot.User.ValidSignature And Ref.WhiteList.Contains(xSpot.User.Modulus), "True", "False")).Replace("[SN:WEB]", Utils.SafeHref(xSpot.Web).Replace("[SN:", "[SN:]")).Replace("[SN:SUBCATS]", Utils.HtmlEncode(xSpot.SubCats).Replace("[SN:", "[SN:]")).Replace("[SN:TITLE]", Utils.HtmlEncode(xSpot.Title).Replace("[SN:", "[SN:]")).Replace("[SN:STATS]", "").Replace("[SN:IMGX]", CStr(xSpot.ImageWidth)).Replace("[SN:IMGY]", CStr(xSpot.ImageHeight)).Replace("[SN:INFO]", MakeTable(xSpot)).Replace("[SN:DESC]", MakeDesc(xSpot))
    ...
    Friend Shared Function ParseComment(ByVal xComment As Comment, ByVal sClass As String, ByVal sTooltip As String) As String
        ...
        If CommentHTM Is Nothing Then
            ...
            Dim sR As New StreamReader(Tools.CommentTheme)
        ...
        Return CommentHTM.Replace("[SN:AVATAR]", GetAvatar(xComment.User.Avatar, xComment.User.Modulus)).Replace("[SN:CLASS]", sClass).Replace("[SN:TOOLTIP]", sTooltip).Replace("[SN:STAMP]", CStr(CType(xComment.Created - Utils.EPOCH, TimeSpan).TotalSeconds)).Replace("[SN:DATE]", xComment.Created.ToLocalTime.ToString("%d MMM | HH:mm")).Replace("[SN:MODULUS]", xComment.User.Modulus).Replace("[SN:PATH]", Tools.SettingsFolder.Replace(Chr(34), Chr(34) & Chr(34))).Replace("[SN:FROM]", Utils.HtmlEncode(xComment.From)).Replace("[SN:UNIQUE]", Spotz.MakeUnique(xComment.User.Modulus)).Replace("[SN:WHITELIST]", Utils.sIIF(Ref.WhiteList.Contains(xComment.User.Modulus), "True", "False")).Replace("[SN:DESC]", MakeBody(xComment))
    ...
    Private Shared Function DefaultAvatar(ByVal sModulus As String) As String
        ...
        Return "http://www.gravatar.com/avatar/" & Utils.MakeMD5(sModulus) & "?s=32&d=identicon"
    ...
    Friend Shared Function MakeTable(ByVal xSpot As Spotlib.SpotEx) As String
        ...
        If Not xSpot.OldInfo Is Nothing Then
            xTable.Append("<TR><TD>&nbsp;</TD><TD>&nbsp;</TD></TR>")
            xTable.Append("<TR><TD><b>Bestandsnaam&nbsp;&nbsp;&nbsp;</b></TD><TD><B><A onfocus='this.blur()' TITLE='Zoeken naar NZB' HREF='link:" & MakeNZBSearch(Utils.HtmlDecode(xSpot.OldInfo.FileName)) & "'>" & Utils.HtmlEncode(xSpot.OldInfo.FileName) & "</A></B></TD></TR>")
        End If
    ...
    Private Shared Function MakeCats(ByVal hCat As Byte, ByVal sCat As Char, ByVal TheCats As Dictionary(Of String, String), ByVal sColor As String, ByVal sColor2 As String) As String
        ...
        If TheCats.Count > 0 Then
            sApp = vbNullString
            xRet.Append("<TR><TD><b>" & Spotz.TranslateCatDesc(hCat, sCat) & "</b>&nbsp;&nbsp;&nbsp;</TD><TD>")
            For Each sG As String In TheCats.Keys
                If hCat = 1 Or hCat = 6 Then
                    xRet.Append(sApp & CreateClassLink("query:" & Utils.URLEncode(TheCats.Item(sG) & "_" & "cats MATCH '" & Utils.StripChars("1" & sG) & " OR " & Utils.StripChars("6" & sG) & "'"), TheCats.Item(sG), "category", "Zoeken") & "</TD></TR>")
                Else
                    xRet.Append(sApp & CreateClassLink("query:" & Utils.URLEncode(TheCats.Item(sG) & "_" & "cats MATCH '" & Utils.StripChars(CStr(hCat) & sG) & "'"), TheCats.Item(sG), "category", "Zoeken") & "</TD></TR>")
                End If
                sApp = "<TR>&nbsp;<TD></TD><TD>"
            Next
        End If
    ...
    Private Shared Function MakeNZBSearch(ByVal sTitle As String) As String
        ...
        Return "http://binsearch.info/index.php?q=" & Utils.URLEncode(sTitle) & Utils.HtmlEncode("&max=250&adv_age=&server=")
    ...
    Private Shared Function CreateClassLink(ByVal sTarget As String, ByVal sText As String, ByVal sClass As String, Optional ByVal sTooltip As String = "") As String
        ...
        Return "<A CLASS='" & sClass & "' " & Utils.sIIF(Len(sTooltip) > 0, " TITLE='" & sTooltip.Replace("'", "''") & "'", " ") & " onfocus='this.blur()' HREF='" & sTarget.Replace("'", "''") & "'>" & Utils.HtmlEncode(sText) & "</A>"
    ...
    Private Shared Function GetCatInfo(ByVal hCat As Byte, ByVal Cats As String, ByVal zTags As String, ByRef zGroups As String, ByVal sPoster As String, ByVal zUser As UserInfo) As String
        ...
        If CatGroups.Count > 0 Then
            sApp = vbNullString
            zGroups = "<TR><TD><b>Nieuwsgroep" & Utils.sIIF(CatGroups.Count > 1, "en", "") & "</b></TD><TD>"
            For Each sG As String In CatGroups
                If sG <> "Other" Then
                    zGroups += sApp & Utils.HtmlEncode(sG) & "</TD></TR>"
                    sApp = "<TR><TD>&nbsp;</TD><TD>"
                End If
            Next
    ...
    Friend Shared Function InsertSmileys(ByVal sHTML As String) As String
        ...
        Do While sHTML.Contains("[img=")
            ...
            If (sp >= 0 And sp2 > 0) And ((sp2 - sp) > 5) Then
                ...
                Dim sSmile As String = Utils.StripChars(sHTML.Substring(sp + 5, sp2 - (sp + 5)))

                sHTML = sHTML.Remove(sp, sp2 - (sp - 1))

                Dim sFile As String = Tools.SettingsFolder() & "\Images\Smileys\" & sSmile & ".gif"
                If Utils.FileExists(sFile) Then sHTML = sHTML.Insert(sp, "<IMG onfocus='this.blur()' title=" & Chr(34) & sSmile & Chr(34) & " SRC=" & Chr(34) & sFile & Chr(34) & ">")
    ...
    Friend Shared Function Linkify(ByVal sIn As String) As String
        ...
        For Each xMatch As Match In Matches
            ...
            sIn = sIn.Replace(xMatch.Value, "<A onfocus='this.blur()' TITLE='Link openen' HREF=" & Chr(34) & "link:" & Utils.SafeHref(Utils.HtmlDecode(xMatch.Value)) & Chr(34) & ">" & xMatch.Value & "</A>")
    ...
    Private Shared Function MakeGoogleSearch(ByVal sTitle As String) As String
        ...
        Return "http://www.google.nl/search?q=" & Utils.URLEncode(sTitle)
    ...
    Private Shared Function SHA1(ByVal sVal As String) As String
        ...
        Return Utils.StripChars(Convert.ToBase64String(Kl.ComputeHash(Utils.MakeLatin(sVal))))
